...
def load_index(root: str) -> pd.DataFrame:
    ...
    # Better reject_flag filtering for string/bool types
...
def compute_data_hash(df: pd.DataFrame) -> str:
    """Compute hash of participant IDs and paths for cache validation."""
...
def build_all(root: str, df: pd.DataFrame, fs_default: int, most_ch: int, quick: bool = False, quick_n_per_subj: int = 50) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    - v2 part is left as-is (global scaler is applied later in the ML pipeline).
    """
    cdir = cache_dir(root)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train classical ML models on 1000 Hz EEG features with subject-wise splitting (window-aware option included)"
    )
    ...
    parser.add_argument('--quick', action='store_true',
                        help='Quick mode for feature extraction (subset per subject)')
    parser.add_argument('--quick_n_per_subj', type=int, default=50,
                        help='Samples per subject in quick mode')
    ...
    parser.add_argument('--window-aware', action='store_true',
                        help='If set, append one-hot window labels (baseline/erp/post) to features (recommended).')
    ...
    print(f"Window-aware ML: {args.window_aware}")
    ...
    # ---------------- Window-aware ML (Option A) ----------------
    window_classes = None
    if args.window_aware:
        if window_labels is None or len(window_labels) != len(X_all):
            raise RuntimeError("window_labels missing/misaligned. Ensure build_all returns window_labels aligned to X.")
        # sklearn version-safe OneHotEncoder
        try:
            win_enc = OneHotEncoder(sparse_output=False, handle_unknown="ignore")
        except TypeError:
            win_enc = OneHotEncoder(sparse=False, handle_unknown="ignore")

        W = win_enc.fit_transform(window_labels.reshape(-1, 1)).astype(np.float32)
        window_classes = [str(x) for x in win_enc.categories_[0].tolist()]
        X_all = np.concatenate([X_all, W], axis=1).astype(np.float32)

        print("\n✓ Window-aware enabled (one-hot appended)")
        print("  Window classes:", window_classes)
        print("  New X_all shape:", X_all.shape)
    # ------------------------------------------------------------
    ...
    for name, pipe in models.items():
        ...
        for fold_idx, (tr_in, va_in) in enumerate(tqdm(fold_iter, total=cv.get_n_splits(), desc=f"{name} CV", unit="fold"), start=1):
...
"""
....aware one
cd /home/asatsan2/Projects/EEG-Pain-Estimation
touch train_ml.log

screen -S painjob1 -dm bash -c "
source ~/.bashrc
conda activate eeg
python scripts/new_ml_aware.py \
  --data-root data \
  --feature-set all \
  --test-size 0.25 \
  --seed 42 \
  --models RandomForest LogReg_bal LinearSVC_bal XGBoost \
  --window-aware \
  >> train_ml.log 2>&1
"
...
Now, not aware ones:
touch train_ml_without_aware.log

screen -S painjob2 -dm bash -c "
source ~/.bashrc
conda activate eeg
python scripts/new_ml_aware.py \
  --data-root data \
  ...
  --seed 42 \
  --models RandomForest LogReg_bal LinearSVC_bal XGBoost \
  >> train_ml_without_aware.log 2>&1
"

train_ml_withot_aware.
screen -r painjob1 - to arttach 
ctrl A+D to remove from screen 
ctrl c for remove from tail .. commad: tail -f train_ml.log
    """