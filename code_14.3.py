...
class WindowAware_DeepCNN_LSTM(nn.Module):
    def __init__(
        ...
        k_ch = max(1, n_channels // 8)

        self.conv1 = nn.Conv2d(
            1, cnn_filters[0], kernel_size=(k_ch, 3), padding=(0, 1)
        )
    ...
    def forward(self, x, window_idx):
        w_emb = self.window_embedding(window_idx)
        ...
        x = torch.cat([x, w_exp], dim=2)

        _, (h, _) = self.lstm(x)
        h = torch.cat([h[-2], h[-1]], dim=1)

        h = torch.cat([h, w_emb], dim=1)
        h = self.dropout(h)
        return self.fc(h)
...
# =============================================================================
# GRAD-CAM + CHANNEL/TIME IMPORTANCE
# =============================================================================

class LayerGradCAM:
    """
    Generic Grad-CAM for a chosen conv layer.
    Returns:
      - conv-space CAM
      - resized channel×time CAM on original EEG size
      - channel importance
      - time importance
    """
...
def resize_cam_to_input(cam_2d: np.ndarray, target_channels: int, target_time: int) -> np.ndarray:
    """
    cam_2d: (H, W)
    returns: (target_channels, target_time)
    """
    cam_tensor = torch.tensor(cam_2d, dtype=torch.float32)[None, None, :, :]
    cam_resized = F.interpolate(
        cam_tensor,
        size=(target_channels, target_time),
        mode="bilinear",
        align_corners=False,
    )
...
def get_channel_time_importance(cam_input_space: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
    cam_input_space: (C, T)
    returns:
      channel_importance: (C,)
      time_importance: (T,)
    """
    channel_importance = cam_input_space.mean(axis=1)
    time_importance = cam_input_space.mean(axis=0)

    channel_importance = (channel_importance - channel_importance.min()) / (
        channel_importance.max() - channel_importance.min() + 1e-8
    )
    time_importance = (time_importance - time_importance.min()) / (
        time_importance.max() - time_importance.min() + 1e-8
    )
    return channel_importance, time_importance
...
def save_cam_outputs(
    ...
    raw_eeg: np.ndarray,
    cam_conv: np.ndarray,
    cam_input_space: np.ndarray,
    channel_importance: np.ndarray,
    time_importance: np.ndarray,
):
    os.makedirs(save_dir, exist_ok=True)

    np.save(os.path.join(save_dir, f"{layer_name}_cam_conv.npy"), cam_conv)
    np.save(os.path.join(save_dir, f"{layer_name}_cam_input_space.npy"), cam_input_space)
    np.save(os.path.join(save_dir, f"{layer_name}_channel_importance.npy"), channel_importance)
    np.save(os.path.join(save_dir, f"{layer_name}_time_importance.npy"), time_importance)

    # Conv-space heatmap
    plt.figure(figsize=(8, 4))
    plt.imshow(cam_conv, aspect="auto", cmap="jet")
    plt.colorbar()
    plt.title(f"Grad-CAM - {layer_name} (conv space)")
    plt.xlabel("Conv-time")
    plt.ylabel("Conv-channel")
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"{layer_name}_cam_conv.png"), dpi=300)
    plt.close()

    # Input-space channel×time heatmap
    plt.figure(figsize=(10, 5))
    plt.imshow(cam_input_space, aspect="auto", cmap="jet")
    plt.colorbar()
    plt.title(f"Grad-CAM - {layer_name} (projected to EEG channel×time)")
    plt.xlabel("Time samples")
    plt.ylabel("EEG channels")
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"{layer_name}_cam_input_space.png"), dpi=300)
    plt.close()

    # Time importance overlaid on raw EEG channel-0
    plt.figure(figsize=(10, 4))
    plt.plot(raw_eeg, label="Raw EEG (channel 0)", alpha=0.7)
    plt.plot(time_importance * np.max(np.abs(raw_eeg)), label=f"{layer_name} time importance", linewidth=2)
    plt.legend()
    plt.title(f"Time Importance - {layer_name}")
    plt.xlabel("Time samples")
    plt.ylabel("Amplitude / importance")
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"{layer_name}_time_overlay.png"), dpi=300)
    plt.close()

    # Channel importance bar
    plt.figure(figsize=(10, 4))
    plt.bar(np.arange(len(channel_importance)), channel_importance)
    plt.title(f"Channel Importance - {layer_name}")
    plt.xlabel("Channel index")
    plt.ylabel("Importance")
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"{layer_name}_channel_importance.png"), dpi=300)
    plt.close()

    # Time importance line
    plt.figure(figsize=(10, 4))
    ...
    plt.ylabel("Importance")
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"{layer_name}_time_importance.png"), dpi=300)
    plt.close()
...
def run_window_aware_interpretability(
    ...
    raw_eeg = x0[0, 0].detach().cpu().numpy()
    orig_channels = x0.shape[1]
    orig_time = x0.shape[2]

    # Window embeddings
    w_emb = model.window_embedding.weight.detach().cpu().numpy()
    np.save(os.path.join(save_dir, "window_embeddings.npy"), w_emb)
    ...
    plt.title("Window Embeddings")
    plt.xlabel("Embedding dimension")
    plt.ylabel("Window type")
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "window_embedding_heatmap.png"), dpi=300)
    plt.close()
    print("✓ Window embeddings saved")
    ...
    for layer_name, layer_module in target_layers.items():
        print(f"\n→ Processing Grad-CAM for {layer_name}")

        gradcam = LayerGradCAM(model, layer_module)
        cam_batch, pred_class_idx = gradcam.generate(x0, w0)
        cam_conv = cam_batch[0]

        cam_input_space = resize_cam_to_input(cam_conv, orig_channels, orig_time)
        channel_importance, time_importance = get_channel_time_importance(cam_input_space)

        top_channels_idx = np.argsort(channel_importance)[::-1][:top_k_channels]
        top_times_idx = np.argsort(time_importance)[::-1][:top_k_times]
        ...
        save_cam_outputs(
            ...
            raw_eeg=raw_eeg,
            cam_conv=cam_conv,
            cam_input_space=cam_input_space,
            channel_importance=channel_importance,
            time_importance=time_importance,
        )

        print(f"  Predicted class: {pred_class_idx} ({layer_name})")
        print(f"  Top channels ({layer_name}): {top_channels_idx.tolist()}")
        print(f"  Top times ({layer_name}): {top_times_idx.tolist()}")

        layer_results[layer_name] = {
            "predicted_class_index": int(pred_class_idx),
            "top_channels": top_channels_idx.tolist(),
            "top_channel_scores": channel_importance[top_channels_idx].tolist(),
            "top_times": top_times_idx.tolist(),
            "top_time_scores": time_importance[top_times_idx].tolist(),
            "mean_channel_importance": float(channel_importance.mean()),
            "mean_time_importance": float(time_importance.mean()),
        }

    # Save summary JSON
    with open(os.path.join(save_dir, "interpretability_summary.json"), "w") as f:
        json.dump(layer_results, f, indent=2)

    print(f"\n✓ Interpretability files saved in: {save_dir}")
    print("✓ Includes first-layer and last-layer channel/time importance")
...
def main():
    ...
    ap.add_argument("--interpret_dir", type=str, default="interpretability_outputs")
    args = ap.parse_args()
    ...
    interpret_results = run_window_aware_interpretability(
        model,
        test_loader,
        device,
        window_le,
        save_dir=os.path.join(proj_root, args.interpret_dir),
    )
        ...
        json.dump(
            {
                ...
                "results": metrics,
                "interpretability": interpret_results,
            },
            f,
            indent=2,
        )
...
if __name__ == "__main__":
    ...
    """
    python  /home/asatsan2/Projects/EEG-Pain-Estimation/code_14.3.py
